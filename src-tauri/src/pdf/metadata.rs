...
pub fn remove_javascript(doc: &mut Document) -> Result<CleanResult, String> {
  ...
  let (names_ref, js_tree_id) = if let Ok(Object::Dictionary(catalog)) = doc.get_object(catalog_id) {
  ...
  // 收集 Catalog 中需要处理的数据
  let (has_js, has_aa, should_remove_open_action) = if let Ok(Object::Dictionary(catalog)) = doc.get_object(catalog_id) {
...
pub fn remove_attachments(doc: &mut Document) -> Result<CleanResult, String> {
  ...
  let (names_ref, ef_tree_id) = if let Ok(Object::Dictionary(catalog)) = doc.get_object(catalog_id) {
    let names_id = match catalog.get(b"Names") {
      Ok(Object::Reference(id)) => Some(*id),
      _ => None,
    };
...
pub fn set_redaction_metadata(doc: &mut Document) -> Result<(), String> {
  ...
  if let Ok(Object::Dictionary(ref mut info_dict)) = doc.get_object_mut(info_id) {
    // 设置 Producer（生成工具）
    info_dict.set(b"Producer", Object::String(producer.as_bytes().to_vec(), lopdf::StringFormat::Literal));

    // 设置 Creator（创建程序）
    info_dict.set(b"Creator", Object::String(creator.as_bytes().to_vec(), lopdf::StringFormat::Literal));

    // 设置 ModDate（修改时间）
    info_dict.set(b"ModDate", Object::String(pdf_date.as_bytes().to_vec(), lopdf::StringFormat::Literal));

    // 添加自定义字段标记已脱敏
    info_dict.set(b"Redacted", Object::String(b"true".to_vec(), lopdf::StringFormat::Literal));
    info_dict.set(b"RedactedBy", Object::String(producer.as_bytes().to_vec(), lopdf::StringFormat::Literal));
    info_dict.set(b"RedactedAt", Object::String(pdf_date.as_bytes().to_vec(), lopdf::StringFormat::Literal));
  }

  log::info!("已设置脱敏元信息: Producer={}, ModDate={}", producer, pdf_date);
  Ok(())
}
