use lopdf::{Object, content::{Content, Operation}};
...
fn estimate_text_width(text: &[u8], font_size: f32) -> f32 {
  text.iter().map(|&b| estimate_char_width(b, font_size)).sum()
}
...
fn char_in_mask(char_x: f32, char_y: f32, char_width: f32, font_size: f32, masks: &[MaskRect]) -> bool {
  ...
  masks.iter().any(|m| m.intersects_text_bbox(char_x, char_y, char_width, char_height))
}
  ...
  for op in content.operations {
    ...
    match operator {
      ...
      "Tj" if in_text_object => {
        ...
        let (text_bytes, str_format) = if let Some(Object::String(s, fmt)) = op.operands.first() {
          (s.clone(), *fmt)
        } else {
          (vec![], lopdf::StringFormat::Literal)
        };

        let (redacted_text, any_redacted) = redact_text_chars(&text_bytes, user_x, user_y, font_size, masks);

        if any_redacted {
          log::info!("[Tj脱敏] {:?} -> {:?}",
            String::from_utf8_lossy(&text_bytes),
            String::from_utf8_lossy(&redacted_text));
          new_operations.push(Operation::new("Tj", vec![Object::String(redacted_text, str_format)]));
        } else {
          new_operations.push(op);
        }
      }
      "TJ" if in_text_object => {
        ...
        if let Some(Object::Array(arr)) = op.operands.first() {
          for item in arr {
            match item {
              Object::String(s, fmt) => {
                let (redacted, redacted_this) = redact_text_chars(s, current_x, user_y, font_size, masks);
                if redacted_this {
                  any_redacted = true;
                  log::info!("[TJ脱敏] {:?} -> {:?}",
      ...
      "'" if in_text_object => {
        ...
        if any_redacted {
          log::info!["['脱敏] {:?} -> {:?}",
            String::from_utf8_lossy(&text_bytes),
            String::from_utf8_lossy(&redacted_text)];
          new_operations.push(Operation::new("'", vec![Object::String(redacted_text, str_format)]));
        } else {
          new_operations.push(op);
        }
      }
...
pub fn add_black_overlay(content_data: &[u8], masks: &[MaskRect]) -> Result<Vec<u8>, String> {
  ...
  for rect in masks {
    new_operations.push(Operation::new("re", vec![
      Object::Real(rect.x),
      Object::Real(rect.y),
